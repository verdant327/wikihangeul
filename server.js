...
function getLeaderboard(limit = null) {
  if (useJsonFallback) {
    const validUsers = (jsonStore.users || []).filter(u => u && u.nickname && !isProhibitedNickname(u.nickname).prohibited);
...
function recordWrongAnswer(userId, quizId, userAnswer, correctAnswer) {
  ...
  try {
    const delStmt = db.prepare('DELETE FROM wrong_answers WHERE user_id = ? AND quiz_id = ?');
    delStmt.run(userId, quizId);
  } catch (e) {}
...
function getWrongAnswers(userId, limit = 50) {
  ...
  const stmt = db.prepare(`
    SELECT * FROM wrong_answers WHERE user_id = ? ORDER BY id DESC LIMIT ?
  `);
  return stmt.all(userId, limit);
}
...
function restoreOrSyncUser(userData, leaderboardSnapshot = []) {
  ...
  if (isProhibitedNickname(cleanNick).prohibited) {
    console.log(`[DB-Sync] 🚫 Rejected sync of prohibited user: ${cleanNick}`);
    return null;
  }
    ...
    if (needsUpdate || (existing.password === 'saved_user' && userData.password && userData.password !== 'saved_user')) {
      if (useJsonFallback) {
        ...
        if (idx !== -1) {
          ...
          if (existing.password === 'saved_user' && userData.password && userData.password !== 'saved_user') {
            jsonStore.users[idx].password = userData.password;
          }
          saveJsonStore();
        }
      } else {
        if (existing.password === 'saved_user' && userData.password && userData.password !== 'saved_user') {
          const stmt = db.prepare(`UPDATE users SET rp = ?, wins = ?, losses = ?, draws = ?, password = ? WHERE id = ?`);
          stmt.run(newRp, newWins, newLosses, newDraws, userData.password, existing.id);
        } else {
