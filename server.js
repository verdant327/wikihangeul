...
const FALLBACK_QUIZZES = [
    ...
    {
        "id":  191,
        "question":  "다음 중 올바른 표기는 무엇일까요?\n\"갑자기 뒤에서 큰 소리로 친구를 깜짝 ( ).\"",
        "options":  [
                        "놀라게 했다",
                        "놀래켰다"
                    ],
        "answer":  "놀라게 했다",
        "explanation":  "남을 놀라게 만드는 것은 \u0027놀라게 하다\u0027 또는 \u0027놀래다\u0027가 표준어입니다. \u0027놀래키다(놀래켰다)\u0027는 비표준어이므로 \u0027놀라게 했다\u0027가 바른 표기입니다.",
        "source":  "바른 국어 맞춤법"
    },
    {
        "id":  192,
    ...
    },
    {
        ...
        "answer":  "식해",
        "explanation":  "생선(가자미 등)을 곡식과 함께 삭힌 발효 반찬은 \u0027식해(食醢)\u0027이고, 명절에 마시는 밥알이 뜬 달콤한 전통 음료는 \u0027식혜(食醯)\u0027입니다.",
        "source":  "국립국어원 표준국어대사전",
        "id":  288
    },
    {
...
const server = http.createServer((req, res) => {
  ...
  if (pathname === '/api/admin/leaderboard-export' && req.method === 'GET') {
    const list = db.getLeaderboard();
    res.writeHead(200, {
      'Content-Type': 'application/json; charset=utf-8',
      'Content-Disposition': 'attachment; filename="waterpang_s3_leaderboard_backup.json"'
    });
    return res.end(JSON.stringify({ ok: true, season: 3, exportedAt: new Date().toISOString(), leaderboard: list }, null, 2));
  }

  if (pathname === '/api/admin/leaderboard-import' && req.method === 'POST') {
    parseJsonBody((err, body) => {
      if (err || !body || !Array.isArray(body.leaderboard)) {
        res.writeHead(400, { 'Content-Type': 'application/json; charset=utf-8' });
        return res.end(JSON.stringify({ error: '올바른 랭킹 데이터(JSON 배열)를 제공해주세요.' }));
      }
      let importedCount = 0;
      for (const item of body.leaderboard) {
        if (item && item.nickname) {
          db.restoreOrSyncUser(item);
          importedCount++;
        }
      }
      res.writeHead(200, { 'Content-Type': 'application/json; charset=utf-8' });
      return res.end(JSON.stringify({ ok: true, message: `성공적으로 ${importedCount}명의 랭킹 데이터를 복원했습니다!`, count: importedCount }));
    });
    return;
  }
    ...
    parseJsonBody((err, body) => {
      ...
      if (reqNick === db.TEACHER_ACCOUNT.nickname || reqNick.replace(/\s+/g, '') === '하하하하하쌤') {
        if (reqPw === db.TEACHER_ACCOUNT.password) {
          db.ensureTeacherAccount();
          if (body.backupUser) {
            db.restoreOrSyncUser(body.backupUser, body.leaderboardSnapshot);
          }
          const teacher = db.getUserByNickname(db.TEACHER_ACCOUNT.nickname) || {
      ...
      // ALWAYS merge and elevate client backupUser to prevent server restart downgrade!
      if (body.backupUser && body.backupUser.nickname === reqNick) {
        user = db.restoreOrSyncUser(body.backupUser, body.leaderboardSnapshot);
      }
  ...
  if (pathname === '/api/profile' && req.method === 'GET') {
    ...
    if (!user) {
      user = db.getUserByNickname(userId);
    }
    if (!user) {
      res.writeHead(404, { 'Content-Type': 'application/json; charset=utf-8' });
  ...
  if (pathname === '/api/leaderboard' && req.method === 'GET') {
    // Return all students with zero-cache to ensure Phone & PC always see identical real-time rankings
    const list = db.getLeaderboard();
    sendOptimized(req, res, 200, {
      'Content-Type': 'application/json; charset=utf-8',
...
server.listen(PORT, '0.0.0.0', () => {
  ...
  // Auto Keep-Alive: Prevents cloud containers (Koyeb/Render) from sleeping due to inactivity
  const KEEP_ALIVE_INTERVAL = 8 * 60 * 1000; // 8 minutes
  setInterval(() => {
    try {
      const host = process.env.KOYEB_APP_NAME ? `${process.env.KOYEB_APP_NAME}.koyeb.app` : (process.env.RENDER_EXTERNAL_HOSTNAME || process.env.PUBLIC_URL || null);
      if (host) {
        const https = require('node:https');
        const cleanHost = host.replace(/^https?:\/\//, '');
        https.get(`https://${cleanHost}/api/health`, (r) => {}).on('error', () => {});
      }
    } catch (e) {}
  }, KEEP_ALIVE_INTERVAL);
});
