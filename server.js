...
// Game Room Class
// -------------------------------------------------------------
// Global recent questions ring buffer (prevents duplicate questions across consecutive matches)
...
class Room {
  ...
  endMatch() {
    ...
    setTimeout(() => { activeRooms.delete(this.id); }, 2000);
  }
}
...
const server = http.createServer((req, res) => {
  ...
  if (pathname === '/api/events') {
    ...
    req.on('close', () => {
      clearInterval(heartbeat);
      // Only delete if this specific connection record was the active one!
  ...
  if (pathname === '/api/register' && req.method === 'POST') {
    parseJsonBody((err, body) => {
      ...
      try {
        ...
        // Auto-restore stats if student had prior progress in browser backup
        let user;
  ...
  if (pathname === '/api/login' && req.method === 'POST') {
    parseJsonBody((err, body) => {
      ...
      // Container reboot recovery: restore user and leaderboard if client passes backup
  ...
  // Dual-channel guaranteed match status polling endpoint
  ...
  // 3.5 Game State Polling Endpoint (Dual-channel fallback for strict firewalls)
